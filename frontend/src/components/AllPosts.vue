<template>
  <div class="allPosts container">
    ...
    <div class="allPosts__ReadMoreButton">
      <button @click="loadMore(2)" class="readMore__YellowButton" v-if="!isDisabled">
        Load More
      </button>
    </div>
  </div>
...
<script setup>
  import { onMounted, ref } from 'vue';
    ...
    function loadMore(howMany) {
      ...
      JSON.parse(JSON.stringify(props.data.slice(start, end))).forEach(element => {
...
.allPosts {
  ...
  h1 {
    ...
    font-weight: 700;
    padding-bottom: r(32);
    border-bottom: 1px solid rgba(109, 110, 118, .32);
  }
}
